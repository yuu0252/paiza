...
def dijkstra(h, w, t, sx, sy, gx, gy, n):
    open_list = []
    closed = set()
    ...
    if n > 0:
        initial_state = State(sx, sy, n - 1, 0)
        heapq.heappush(open_list, initial_state)
    ...
    while open_list:
        ...
        for i in  range(4):
            ...
            heapq.heappush(open_list, State(nx, ny, st.ticket, st.cost + t[ny][nx]))
            if st.ticket > 0:
                heapq.heappush(open_list, State(nx, ny, st.ticket - 1, st.cost))
